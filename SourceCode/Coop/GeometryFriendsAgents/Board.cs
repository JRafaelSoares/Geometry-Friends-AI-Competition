namespace GeometryFriendsAgents
{
    ...
    internal class Board
    {
        ...
        protected float[] tempObstacle;
        protected float[] tempDiamonds;
        ...
        public node checkStartNode(float[] SqaureOrCircle_Info)
        {
            top_limit = 5 - ((int) Math.Ceiling((double) (((double) SqaureOrCircle_Info[4]) / 40.0)));
            return new node { 
                x = ((int) SqaureOrCircle_Info[0]) / cellMinSize,
                y = ((int) (SqaureOrCircle_Info[1] - (SqaureOrCircle_Info[4] / 2f))) / cellMinSize
            };
        }
        ...
        public void getBoardValue(float[] obstaclesInfo)
        {
            ...
            if (length > 0)
            {
                ...
                {
                    float x = this.tempObstacle[i];
                    float y = this.tempObstacle[i + 1];
                    float height = this.tempObstacle[i + 2];
                    float width = this.tempObstacle[i + 3];
        ...
        public void getUpdateEndNode(float[] collectiblesInfo)
        {
            ...
            if (length >= 1)
            {
                ...
                {
                    int num2 = ((int) collectiblesInfo[i]) / cellMinSize;
                    int index = ((int) collectiblesInfo[i + 1]) / cellMinSize;
                    if (!AStar.Nodes[num2][index].semi_block)
                    {
                        AStar.endNodes.Add(new int[] { num2, index });
                        AStar.Nodes[num2][index].obstacle = false;
                    }
                }
            }
        }
        ...
        {
            int num = this.get_DyNode(squareInfo[1], squareInfo[4], true);
            int num2 = num - 4;
            int num3 = 0;
            while (num3 < xCellsNum)
            {
                int index = 1;
                while (true)
                {
                    if (index >= num2)
                    {
                        ...
                        while (true)
                        {
                            ...
                            AStar.Nodes[num3][num5].semi_block = true;
                            num5++;
                        }
                        break;
                    }
                    AStar.Nodes[num3][index].semi_block = true;
                    index++;
                }
            }
        }
        ...
        public void UpdateNew(int end_x, int end_y)
        {
            ...
            while (num < xCellsNum)
            {
                ...
                while (true)
                {
                    ...
                    AStar.Nodes[num][index] = (node[]) node;
                    index++;
                }
            }
        }
