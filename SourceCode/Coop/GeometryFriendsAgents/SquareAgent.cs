namespace GeometryFriendsAgents
{
    ...
    internal class SquareAgent : ISquareAgent, IAgent
    {
        ...
        private int COOP_state;
        private bool target_lost = true;
        private int B_count;
        private float COOP_X;
        private float COOP_Y;
        ...
        private float _PREDICT(float vec) => 
            vec;
        ...
        {
            float single1 = 5000f / this.squareInfo[4];
            this.right = this.get_RxNode() + 1;
            this.left = this.get_LxNode() - 1;
        ...
        private void Coor_Action()
        {
            if (this.A_count_check)
            {
                this.Sqaure_Action();
            }
            else
            {
                ...
                switch (this.COOP_state)
                {
                    case 0:
                        this.old_nC = this.nCollectiblesLeft;
                        if (!instance.IsBow())
                        {
                            break;
                        }
                    ...
                    case 1:
                        if (!instance.Isping())
                        {
                            if ((this.reach == -1) && !instance.isfusion)
                            {
                                instance.cmd = 4;
                                instance.setPing();
                                this.COOP_state = 8;
                            ...
                            if ((instance.cmd == 0) || !instance.direct)
                            {
                                break;
                            }
                            this.target_lost = false;
                            this.COOP_state = 1;
                            this.COOP_X = instance.X;
                            this.COOP_Y = instance.Y;
                            this.COOP_state = 2;
                        }
                        return;
                    ...
                    case 2:
                    {
                        ...
                        float num = 200f;
                        if (this.circleInfo[0] > this.squareInfo[0])
                        {
                            num = -num;
                        }
                        this.SetAction(this.STAY(this.circleInfo[0] + num, this.squareInfo[2]));
                        if (Math.Abs((float) (this.circleInfo[0] - this.squareInfo[0])) > 300f)
                        {
                            break;
                        }
                        this.B_count = 0;
                    ...
                    case 3:
                        if ((this.circleInfo[1] > this.squareInfo[1]) && (Math.Abs((float) (this.circleInfo[0] - this.squareInfo[0])) < 200f))
                        {
                            this.COOP_state = 9;
                            return;
                        }
                        while (true)
                        {
                            if (!instance.Isping())
                            {
                                ...
                                while (true)
                                {
                                    if ((this.circleInfo[1] - num2) < this.squareInfo[1])
                                    {
                                        this.SetAction(this.STAY(this.circleInfo[0], this.squareInfo[2]));
                                        if (!this.Isriding())
                                        {
                                            break;
                                        }
                    ...
                    case 4:
                    {
                        ...
                        this.SetAction(this.STAY(this.COOP_X, this.squareInfo[2]));
                        if (Math.Abs((float) (this.COOP_X - this.squareInfo[0])) < 20f)
                        {
                            this.COOP_state = 5;
                        }
                        bool flag = false;
                        ...
                        while (true)
                        {
                            if (num3 >= this.nCollectiblesLeft)
                            {
                                if ((((this.squareInfo[0] >= 140f) && (1140f >= this.squareInfo[0])) || ((this.COOP_X >= 140f) && (1140f >= this.COOP_X))) || !flag)
                                {
                                    break;
                                }
                                this.upping();
                            ...
                            float num4 = this.collectiblesInfo[num3 * 2];
                            float num5 = this.collectiblesInfo[(num3 * 2) + 1];
                            if ((Math.Abs((float) (num4 - this.COOP_X)) < 5f) && (Math.Abs((float) (num5 - this.COOP_Y)) < 5f))
                            {
                                flag = true;
                            }
                            num3++;
                    ...
                    case 6:
                        if (this.circleInfo[1] >= this.squareInfo[1])
                        {
                            this.COOP_state = 9;
                        }
                        if (this.old_nC == this.nCollectiblesLeft)
                        {
                            break;
                        }
                        this.old_nC = this.nCollectiblesLeft;
                    ...
                    case 8:
                        ...
                        {
                            instance.cmd = 6;
                            instance.X = this.circleInfo[0];
                            instance.Y = this.circleInfo[1];
                            instance.setPing();
                            this.COOP_state = 0;
                        }
                        break;
                    ...
                    case 9:
                        ...
                        if (this.squareInfo[0] <= 440f)
                        {
                            this.SetAction(this.STAY(this.circleInfo[0] + 300f, this.squareInfo[2]));
                        }
                        else
                        {
                            this.SetAction(this.STAY(this.circleInfo[0] - 300f, this.squareInfo[2]));
        ...
        {
            int num = 10;
            float num2 = this.squareInfo[1] - this.circleInfo[1];
            float num3 = (this.squareInfo[4] / 2f) + this.circleInfo[4];
            return (((num3 - 1f) <= num2) && (num2 <= (num3 + num)));
        }

        private void PathFinder()
        {
            node startNode = this.S_astar.checkStartNode(this.squareInfo);
            this.S_astar.Square_MakeSemi(startNode, this.squareInfo);
            this.S_astar.getUpdateEndNode(this.collectiblesInfo);
            if (AStar.endNodes.Count == 0)
            {
                this.A_count_check = false;
            }
            else
            {
                this.A_count_check = true;
        ...
        private int STAY(float x, float vec)
        {
            float num = (this.squareInfo[0] + this._PREDICT(this.squareInfo[2])) - x;
            return ((num <= 0f) ? ((num >= 0f) ? 0 : Moves.MOVE_RIGHT) : Moves.MOVE_LEFT);
        }
        ...
        private void tprint(float a)
        {
            using (StreamWriter writer = new StreamWriter(@"C:\test\testPrint.txt", true))
            {
                if (a == 1221f)
                {
                    writer.WriteLine("");
        ...
        private void tprint(float a, float b)
        {
            using (StreamWriter writer = new StreamWriter(@"C:\test\testPrint.txt", true))
            {
        ...
        private void tprint(float a, float b, float c)
        {
            using (StreamWriter writer = new StreamWriter(@"C:\test\testPrint.txt", true))
            {
                writer.Write("{0},{1},{2},", a, b, c);
            }
        }

        private void ttprint(string a)
        {
            using (StreamWriter writer = new StreamWriter(@"C:\test\testPrint.txt", true))
            {
                if (a == "ee")
        ...
        public int update_coor(int input) => 
